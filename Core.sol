...
/**
 * @dev https://sheep.money
 * MILK will be halved at each period.
 */
 
contract Fomosheep is StakePool {
...
contract Growingsheep is StakePool {
    IERC20 public yieldToken;
...
contract sheepFactory is Ownable {
  event sheepBorn(address indexed sheep);

  // Create Fomo sheep
  function createFomosheep() public onlyOwner returns (address sheep) {
    Fomosheep pool = new Fomosheep();
    sheep = address(pool);
    emit sheepBorn(sheep);
  }

  function initializeFomosheep(
    address sheep,
    address _stakeToken,
    address _yieldToken,
    uint256 _duration,
    ...
    IERC20(_yieldToken).transfer(sheep, _reward);
  }

  // Create Growing sheep
  function createGrowingsheep() public onlyOwner returns (address sheep)
  {
    Growingsheep pool = new Growingsheep();
    sheep = address(pool);
    emit sheepBorn(sheep);
  }

  function initializeGrowingsheep(
    address sheep,
    address _stakeToken,
    address _yieldToken,
    uint256 _duration,
    ...
    IERC20(_yieldToken).transfer(sheep, total);
  }
